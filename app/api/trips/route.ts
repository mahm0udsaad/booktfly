...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawData = {
      ...
      price_per_seat_one_way: formData.get('price_per_seat_one_way') ? Number(formData.get('price_per_seat_one_way')) : undefined,
    ...
    const { data: trip, error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        price_per_seat_one_way: parsed.data.price_per_seat_one_way || null,
