...
function BookCarContent({ params }: { params: Promise<{ id: string; locale: string }> }) {
  ...
  // Auto-calculate number_of_days when pickup and return dates change
  useEffect(() => {
    if (pickupDate && returnDate) {
      const pickup = parseISO(pickupDate)
      const ret = parseISO(returnDate)
      if (isValid(pickup) && isValid(ret)) {
        const days = differenceInDays(ret, pickup)
        if (days > 0) {
          setValue('number_of_days', days, { shouldValidate: true, shouldDirty: true })
        }
      }
    }
  ...
  const onSubmit = async (data: CarBookingFormData) => {
    ...
    try {
      ...
      router.push(`/${locale}/checkout/${result.bookingId}?type=car`)
    } catch {
      toast({
        title: t('common.error'),
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-32 md:pt-32 lg:pt-36 lg:pb-12 animate-fade-in-up">
        {/* Back button */}
        <button
          onClick={() => router.push(`/${locale}/cars/${carId}`)}
          ...
          <div className="lg:col-span-8 space-y-6 md:space-y-8">
            ...
            <div className="rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 bg-white p-5 md:p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6 relative overflow-hidden">
              ...
              <div className="sm:min-w-[140px] md:min-w-[160px]">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('cars.price_per_day')}</p>
                <p className="text-xl md:text-2xl font-black text-slate-900">{fmt(car.price_per_day)}</p>
              </div>
            </div>

            {/* Booking form */}
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 md:space-y-8" id="car-booking-form">
              {/* Guest info */}
              ...
              <div className="rounded-[1.5rem] md:rounded-[2rem] border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                  ...
                  <div className="space-y-1.5 md:space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger
                        className={cn(
                          ...
                          errors.pickup_date && errorInputClass
                        )}
                      >
                        <span>
                          {pickupDate
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar
                          ...
                          onSelect={(date) => setValue('pickup_date', date ? format(date, 'yyyy-MM-dd') : '', { shouldValidate: true, shouldDirty: true })}
                    ...
                    {errors.pickup_date && (
                      <p className="text-xs font-bold text-destructive mt-1">{errors.pickup_date.message}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5 md:space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger
                        className={cn(
                          ...
                          errors.return_date && errorInputClass
                        )}
                      >
                        <span>
                          {returnDate
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar
                          mode="single"
                          selected={parseDateValue(returnDate)}
                          onSelect={(date) => setValue('return_date', date ? format(date, 'yyyy-MM-dd') : '', { shouldValidate: true, shouldDirty: true })}
                          disabled={(date) => {
                            const minDate = pickupDate ? parseISO(pickupDate) : new Date()
                            return date <= minDate
                          }}
                          initialFocus
                        />
                    ...
                    {errors.return_date && (
                      <p className="text-xs font-bold text-destructive mt-1">{errors.return_date.message}</p>
                    )}
                  </div>
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-sm font-medium text-slate-300">
                    <span>{t('cars.price_per_day')}</span>
                    <span className="font-mono bg-white/10 px-2 py-1 rounded">{fmt(car.price_per_day)}</span>
                  </div>
                  <div className="flex items-center justify-between text-sm font-medium text-slate-300">
